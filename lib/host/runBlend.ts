...
import { hostOutput, type HostOutput, type ItemKind, type ReactionKind } from "@/lib/types";
...
export type BlendPayload = {
  circle: { name: string };
  members: MemberPayload[];
  items: ItemPayload[];
  past_prompts: PastPromptPayload[];
};

type AdminClient = ReturnType<typeof createAdminClient>;

// The app queues blends and an external worker (claude CLI, OpenClaw, Hermes)
// generates them via /api/host/pending + /api/host/complete when set to
// "external". "api" calls Anthropic inline and needs ANTHROPIC_API_KEY.
export function hostRunnerMode(): "api" | "external" {
  return process.env.HOST_RUNNER === "external" ? "external" : "api";
}
...
export async function markBlendFailed(blendId: string, model: string): Promise<void> {
...
export async function gatherBlendPayload(
  admin: AdminClient,
  circleId: string,
): Promise<BlendPayload> {
    ...
    for (const member of members) {
      ...
      for (const row of itemRows ?? []) {
        items.push({
          ...
          excerpt: typeof row.excerpt === "string" ? row.excerpt.slice(0, 280) : null,
...
export async function completeBlend(
  admin: AdminClient,
  args: {
    blendId: string;
    circleId: string;
  ...
  const { blendId, circleId, payload, output, model } = args;

  const validItemIds = new Set(payload.items.map((item) => item.id));
  const validPrompts = output.prompts.filter((prompt) =>
    prompt.evidence.every((entry) => validItemIds.has(entry.item_id)),
  );

  const memberIdByName = new Map(
    payload.members.map((member) => [member.name.trim().toLowerCase(), member.id]),
  );
...
export async function runBlend(
  ...
  // Reuse a recent pending blend instead of piling up queue entries.
  ...
  const { data: existingPending } = await admin
    ...
    .maybeSingle();
  if (existingPending) {
    return { blendId: existingPending.id as string, promptCount: 0, queued: true };
  }
  ...
  if (hostRunnerMode() === "external") {
    // An external worker picks this up via /api/host/pending.
    return { blendId, promptCount: 0, queued: true };
  }

  try {
    const payload = await gatherBlendPayload(admin, circleId);
    ...
    return { blendId, promptCount, queued: false };
  } catch (error) {
    await markBlendFailed(blendId, model);
    throw error;
  }
}
